...
def run(
    base_configuration, benchmark, ignore_error=False, perforation_script: str = None
):
    print(
    ...
    args = "-n {number_cores} -c {config} --benchmarks={benchmark} --no-roi --sim-end=last -senergystats:{periodic} -speriodic-power:{periodic}{script}{perforation}{benchmark_options}".format(
...
def gen_test_data():
    for benchmark in (
        ...
        for freq in (1, 2):
            for parallelism in (min(max_parallelism, NUMBER_CORES),):
...
def gen_training_data():
    for benchmark in (
        ...
        # "parsec-canneal",
        # "parsec-dedup",
        # "parsec-ferret",  ---- no parallelism!
        # "parsec-fluidanimate", ---- infeasible parallelism!
    ...
    for benchmark in (
        ...
        for freq in (4,):
            for parallelism in (4,):
                run(
                    ["{:.1f}GHz".format(freq), "maxFreq", "slowDVFS"],
                    get_instance(benchmark[0], parallelism, input_set="simsmall"),
                    perforation_script="magic_perforation_rate:%s"
                    % ",".join(loop_rates),
                )
